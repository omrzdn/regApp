...
if (isset($_GET['email'], $_GET['reset_token']) && !empty($_GET['email']) && !empty($_GET['reset_token'])) {
  ...
  $stmt = $pdo->prepare('SELECT * FROM users WHERE reset_token = :reset_token AND email = :email');
  $stmt->execute([
  ...
  if ($stmt->rowCount()) {
    ...
    *:focus {
    ...
    <form action="" method="POST">
      <input type="password" id="login" class="fadeIn second" name="new_password" placeholder="Enter your new password" autocomplete="off" style="margin-top: 25px;" required />
      <input type="password" id="password" class="fadeIn third" name="password_confirm" placeholder="repeat your password" autocomplete="off" required />
      <button type="submit" name="submit" class="fadeIn fourth">reset password</button>
    </form>
